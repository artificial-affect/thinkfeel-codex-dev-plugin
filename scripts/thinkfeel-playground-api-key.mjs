...
async function decryptPayloadWithPrivateKey(privateKey, encryptedPayload) {
  ...
  if (typeof encryptedPayload.ciphertext !== 'string') {
    fail('Encrypted API key ciphertext is missing.');
  }
  ...
  const serverReady = new Promise((resolve, reject) => {
    const server = http.createServer(async (request, response) => {
      try {
        ...
        response.end('<!doctype html><title>ThinkFeel Login</title><p>ThinkFeel key setup complete. You can close this tab.</p>');
        server.close();
...
function validateApiKey(value) {
  if (!SAFE_API_KEY_PATTERN.test(value)) {
    fail('Decrypted API key is not a safe ThinkFeel/OpenAI API key literal.');
  }
}
...
function validatePersonaId(value) {
  ...
  if (!/^[A-Za-z0-9._:-]+$/.test(value)) {
    fail('Persona ID contains unsupported characters.');
  }
}
...
async function login(args) {
  ...
  const workspacePath = typeof args.workspace === 'string' && args.workspace.trim() ? args.workspace.trim() : process.cwd();
  const personaId = getPersonaId(args);
  const baseUrl = typeof args['base-url'] === 'string' && args['base-url'].trim() ? args['base-url'].trim() : DEFAULT_BASE_URL;
